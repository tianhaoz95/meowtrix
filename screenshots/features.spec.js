 ...
 *
 * Each feature gets its own fresh browser context with Playwright video
...
// Spin up a fresh recorded page, run the scenario, then convert the recording to
// website/assets/features/<name>.gif.
async function captureFeature(browser, name, scenario) {
  ...
  // Capture every feature in the dark (Midnight) theme for a consistent look
  // across the Features page — matching the site's primary dark aesthetic. The
  // themes capture cycles away from this starting point on its own.
  ...
  convertToGif(webm, path.join(GIF_DIR, `${name}.gif`));
  fs.rmSync(tmpDir, { recursive: true, force: true });
  console.log(`Saved feature GIF: ${name}.gif`);
}
...
test.describe('Generate Feature Showcase GIFs', () => {
  ...
  test('tiling panes & splits', async ({ browser }) => {
    await captureFeature(browser, 'tiling-panes', async (page) => {
  ...
  test('persistent terminal', async ({ browser }) => {
    await captureFeature(browser, 'terminal', async (page) => {
  ...
  test('embedded browser', async ({ browser }) => {
    await captureFeature(browser, 'embedded-browser', async (page) => {
      const pane = page.locator('.pane').first();
  ...
  test('code editor & git', async ({ browser }) => {
    await captureFeature(browser, 'code-editor', async (page) => {
      const pane = page.locator('.pane').first();
      await pane.locator('.tab-add').click();
      const picker = page.locator('.tab-type-picker');
      await expect(picker).toBeVisible();
  ...
  test('command palette', async ({ browser }) => {
    await captureFeature(browser, 'command-palette', async (page) => {
  ...
  test('scheduled enter', async ({ browser }) => {
    await captureFeature(browser, 'scheduled-enter', async (page) => {
      const pane = page.locator('.pane').first();
      ...
      const dialog = page.locator('#sched-dialog, .sched-dialog').first();
      await page.waitForTimeout(900);
  ...
  test('themes', async ({ browser }) => {
    await captureFeature(browser, 'themes', async (page) => {
      ...
      await page.keyboard.press('Enter');
      await page.waitForTimeout(900);
      // Cycle through several themes; setTheme is a shared global action.
      for (const theme of ['ocean', 'matrix', 'ember', 'sakura', 'synthwave', 'dark']) {
