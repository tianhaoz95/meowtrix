...
`install.sh` clones/updates into `~/.meowtrix/app`, installs prod deps, and writes a `meowtrix` launcher to `~/.local/bin`. With `--service` it installs a launchd agent (macOS) or systemd user service (Linux) that auto-starts on login and restarts on crash. The service also sets `MEOWTRIX_SUPERVISED=1` (and uses launchd `KeepAlive` / systemd `Restart=always`) so the in-app self-update can `process.exit(0)` and be relaunched on the new code. The README's install commands curl this script from GitHub.
