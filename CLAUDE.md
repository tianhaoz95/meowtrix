...
Load order in `index.html`: `ws.js` → `pane.js` → `editor.js` → `layout.js` → `app.js` → `mobile.js` → `settings.js` → `palette.js` → `schedule.js`. Functions are shared via the global scope (e.g. `wsSend`, `getAllPanes`, `applyStickyMods`, `runAppShortcut`), so cross-file calls are plain function references, not imports.
